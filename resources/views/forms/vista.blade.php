...
@section('content')
    ...
    <div class="form-container">
        <div class="form-section">
            <h3>SOLICITUD Nº {{$form->id}} de {{$form->nombre}}</h3>

            <div class="estado">
              <label class="negrita">  Estado de la solicitud: </label>
            <span class="{{ $form->estado === 'Abierta' ? 'green' : ($form->estado === 'Cerrado' ? 'grey' : '') }}">

            <i class="fa-solid fa-circle"></i> {{$form->estado}}</span>
            ...
            <div class="form-group-row">
                <div class="form-group">
                    ...
                    <input type="text" id="cargo" name="cargo" value="{{$form->cargo}}" readonly >
                </div>
                <div class="form-group">
                    <label for="fecha" class="negrita">Fecha en la que se realiza la solicitud:</label>
                    <input type="text" id="fecha" name="fecha" value="{{$form->fecha}}" readonly>
                </div>
            </div>
            <div class="form-group">
                <label for="id_tipo_formacion" class="negrita">Tipo de Formacion Solicitada:</label>
                <input type="text" id="id_tipo_formacion" name="id_tipo_formacion"  value="{{ $form->tipo ? $form->tipo->nombre : 'No disponible' }}" readonly >
            </div>
            <div class="form-group">
                ...
                <textarea name="detalle_formacion" id="detalle_formacion" readonly  >{{$form->detalle_formacion}}</textarea>
            </div>
            <div class="form-group">
                ...
                <textarea name="formacion_inicial" id="formacion_inicial" readonly >{{$form->formacion_inicial}}</textarea>
            </div>
            <div class="form-group">
                <label for="observaciones" class="negrita">Detalla a continuación cualquier observación importante para ti:</label>
                <textarea name="observaciones" id="observaciones" readonly >{{$form->observaciones}}</textarea>
            </div>
        </div>
